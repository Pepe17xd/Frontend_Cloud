...
export function OrbitPage() {
  ...
  return <main className="cinema-page"><div className="cinema-topbar"><Link to={`/movie/${movie.id}`}>← Volver a la película</Link><p><i /> Órbita conectada · {watchRoom.status}</p><button disabled>Chat en preparación</button></div><div className="cinema-layout"><div className="cinema-stage"><VideoPlayer movie={movie} streamUrl={session?.stream?.url} streamType={session?.stream?.type} isPlaying={room.isPlaying} onPlayingChange={handlePlayingChange} sessionLoading={isSessionLoading} sessionError={isSessionError}/><CinemaControls isPlaying={room.isPlaying} onTogglePlayback={room.togglePlayback}/></div><aside className="social-panel"><Participants participants={watchRoom.participants} /><section className="chat-ready"><div className="panel-title"><h3>Chat de la órbita</h3><span>Próximamente</span></div><p>La sala está lista para mensajería en tiempo real cuando se conecte WebSocket.</p></section></aside></div></main>;
}
